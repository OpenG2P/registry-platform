...
Copied from the staff portal API, which is the only other service that emits
these. The two are near-identical; the differences are the CloudEvent `source`,
`module` and type prefix, all parameters. TODO: consolidate both into
openg2p_registry_core so there is one implementation to keep correct.

Why the agent portal needs it at all: `g2p_vc_issuances` records that a
credential WAS issued, and Certify's ledger records what it signed, but neither
records the agent's other actions -- who looked a beneficiary up by national ID,
whose authentication was started, what was refused. That is exactly the trail an
investigation needs, and without this middleware none of it left the pod.
...
Disabled by default: set REGISTRY_AGENT_PORTAL_API_AUDIT_ENABLED=true
AND REGISTRY_AGENT_PORTAL_API_AUDIT_MANAGER_URL=<base-url> to turn on.
Set REGISTRY_AGENT_PORTAL_API_AUDIT_ANONYMOUS_FAILURES=false to skip
...
class AuditMiddleware(BaseHTTPMiddleware):
    ...
    def __init__(
        ...
        *,
        ...
        source: str = "/openg2p/registry-agent-portal-api",
        module: str = "registry-agent-portal-api",
        event_type_prefix: str = "org.openg2p.agent_portal",
    ...
    def _build_event(
        ...
        return {
            ...
            "id": str(uuid4()),
            "source": self._source,
            "type": f"{self._event_type_prefix}.{func_name}",
            "time": _now_iso(),
